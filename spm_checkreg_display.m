function spm_checkreg_display(op,varargin)
% image and header display
% FORMAT spm_checkreg_display
%_______________________________________________________________________
%
% spm_checkreg_display is an interactive facility that allows orthogonal sections
...
%
%_______________________________________________________________________
% @(#)spm_checkreg_display.m	2.20 John Ashburner 02/10/29
...
if nargin == 0,
	...
	%-----------------------------------------------------------------------
	P      = spm_get(Inf,'IMAGE','please select image(s)',[],0);
	spm_checkreg_display('init',P);
	return;
end;
...
if strcmp(op,'repos'),
	...
	% spm_orthviews('MaxBB');
	spm_checkreg_display('zoom_in');
	spm_checkreg_display('update_info');
...
if strcmp(op,'addblobs'),
	...
	for i=1:nblobs,
		...
		set(st.blobber,'String','Remove Blobs','Callback','spm_checkreg_display(''rmblobs'');');
	end;
...
if strcmp(op,'rmblobs'),
	...
	set(st.blobber,'String','Add Blobs','Callback','spm_checkreg_display(''addblobs'');');
end;
...
if strcmp(op,'reorient'),
	...
	if sum((tmp(:)-st.vols{1}.mat(:)).^2) > 1e-8,
	  st_tmp = [st.vols{:}];
	  spm_checkreg_display('init',strvcat(st_tmp(:).fname));
	end;
	return;
end;
...
% bit added by MB, from spm_check_registration
mn = length(P);
m  = round(mn^0.4);
n  = ceil(mn/m);
w  = 1/n;
...
for ij=1:mn,
  ...
  j  = w*rem(ij-1,n);
  handle(ij) = spm_orthviews('Image', P(ij),...
...
spm_orthviews('MaxBB');
st.callback = 'spm_checkreg_display(''shopos'');';
...
%-----------------------------------------------------------------------
uicontrol(fg,'Style','Frame','Position',[60 25 200 325].*WS,'DeleteFcn','spm_checkreg_display(''reset'');');
...
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',1)','Position',[175 220 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',2)','Position',[175 200 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',3)','Position',[175 180 065 020].*WS,'String','0','ToolTipString','translate');
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',4)','Position',[175 160 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',5)','Position',[175 140 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',6)','Position',[175 120 065 020].*WS,'String','0','ToolTipString','rotate');
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',7)','Position',[175 100 065 020].*WS,'String','1','ToolTipString','zoom');
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',8)','Position',[175  80 065 020].*WS,'String','1','ToolTipString','zoom');
uicontrol(fg,'Style','edit','Callback','spm_checkreg_display(''repos'',9)','Position',[175  60 065 020].*WS,'String','1','ToolTipString','zoom');

uicontrol(fg,'Style','Pushbutton','String','Reorient images...','Callback','spm_checkreg_display(''reorient'')',...
         'Position',[70 35 125 020].*WS,'ToolTipString','modify position information of selected images');

uicontrol(fg,'Style','Pushbutton','String','Reset...','Callback','spm_checkreg_display(''resetorient'')',...
...
st.mp = uicontrol(fg,'Style','edit', 'Position',[110 295 135 020].*WS,'String','','Callback','spm_checkreg_display(''setposmm'')','ToolTipString','move crosshairs to mm coordinates');
st.vp = uicontrol(fg,'Style','edit', 'Position',[110 275 135 020].*WS,'String','','Callback','spm_checkreg_display(''setposvx'')','ToolTipString','move crosshairs to voxel coordinates');
...
st.zoomer = uicontrol(fg,'Style','popupmenu' ,'Position',[315 75 125 20].*WS,...
	...
	'Callback','spm_checkreg_display(''zoom_in'')','ToolTipString','zoom in by different amounts');
c = 'if get(gco,''Value'')==1, spm_orthviews(''Space''), else, spm_orthviews(''Space'', 1);end;spm_checkreg_display(''zoom_in'')';
...
st.win = uicontrol(fg,'Style','popupmenu','Position',[315 35 125 20].*WS,...
	'String',str2mat('Auto Window','Manual Window'),'Callback','spm_checkreg_display(''window'');','ToolTipString','range of voxel intensities displayed');
% uicontrol(fg,'Style','pushbutton','Position',[315 35 125 20].*WS,...
% 	'String','Window','Callback','spm_checkreg_display(''window'');','ToolTipString','range of voxel intensities % displayed');
st.blobber = uicontrol(fg,'Style','pushbutton','Position',[450 35 125 20].*WS,...
	'String','Add Blobs','Callback','spm_checkreg_display(''addblobs'');','ToolTipString','superimpose activations');
end;
return;
